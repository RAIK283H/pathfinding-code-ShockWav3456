...
class Scoreboard:
    ...
    def __init__(self, batch, group):
        ...
        for index, player in enumerate(config_data.player_data):
            ...
            unique_nodes_visited_label = pyglet.text.Label("Number of Unique Nodes Visited:",
                                                      x=0,
                                                      y=0,
                                                      font_name='Arial',
                                                      font_size=self.font_size, batch=batch, group=group, color=player[2][colors.TEXT_INDEX])

            self.player_unique_nodes_visited_display.append(
                (unique_nodes_visited_label, player))
    ...
    def update_elements_locations(self):
        ...
        for index, (display_element, player) in enumerate(self.player_unique_nodes_visited_display):
            display_element.x = config_data.window_width - self.stat_width
